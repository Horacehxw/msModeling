...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "tools" / "perf_data_collection"))
from discover_operators import discover_operators


def test_m6_key_in_coverage_dict(tmp_path):
    """discover_operators should include m6_profiling_latency_coverage in coverage."""
    ...
    assert "m6_profiling_latency_coverage" in cov
    # MatMulV2 (100us) + SwiGlu (20us) = 120, total = 150 → 80%
    assert abs(cov["m6_profiling_latency_coverage"] - 0.8) < 1e-9
