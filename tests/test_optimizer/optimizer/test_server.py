...
def test_backup_path_exists():
    ...
    with patch("src.optimize.experimental.optimizer.server.Path") as mock_path:
...
def test_run_unknown_cmd():
    ...
    with patch.object(scheduler, 'get_cmd_param', return_value=('unknown_cmd', None)):
        with patch('src.optimize.experimental.optimizer.server.logger') as mock_logger:
            assert scheduler.run() == ''
...
def test_run_no_param():
    ...
    with patch.object(scheduler, 'get_cmd_param', return_value=('command', None)):
        with patch('src.optimize.experimental.optimizer.server.getattr', return_value=scheduler.command):
