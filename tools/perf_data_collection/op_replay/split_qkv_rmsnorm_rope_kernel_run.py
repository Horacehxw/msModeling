...
Notes:
  - This kernel is a vLLM-Ascend Triton fused op, not a public torch_npu API.
  - The current ATLAS_800_A3_752T_128G_DIE/v0.13.0 dataset contains three rows
    with inputs:
    ...
    and outputs:
      * q: (tokens, q_hidden)
      * k: (tokens, kv_hidden)
  - CSV does not record q_weight / k_weight / eps, so this replay follows the
    upstream test configuration and generates per-head BF16 weights with
    eps=1e-6.
"""
...
def infer_head_dim(q_hidden_size: int, kv_hidden_size: int, rope_dim: int) -> int:
    if q_hidden_size <= 0 or kv_hidden_size <= 0 or rope_dim <= 0:
        raise ValueError(
            ...
            f"q_hidden_size={q_hidden_size}, kv_hidden_size={kv_hidden_size}, rope_dim={rope_dim}"
        )
...
def build_argparser():
    return build_standard_argparser(
        ...
        usage_examples=[
            ...
            "python tools/perf_data_collection/op_replay/split_qkv_rmsnorm_rope_kernel_run.py "
...
def run_row(csv_path, row_index: int, row: dict[str, str]) -> None:
    ...
    q_out, k_out, v_out = runtime_torch.ops.vllm.qkv_rmsnorm_rope(
        input=case["qkv_tensor"],
        cos_sin_cache=case["cos_sin_cache"],
        positions=case["positions"],
        q_weight=case["q_weight"],
        k_weight=case["k_weight"],
        q_hidden_size=case["q_hidden_size"],
        kv_hidden_size=case["kv_hidden_size"],
    ...
    print(
        ...
        f"head_dim={case['head_dim']} eps={DEFAULT_EPS}"
    )
...
def main() -> None:
    ...
    ensure_npu_available()
    ensure_vllm_ascend_available()

    target_data_dir = get_target_data_dir(
        device=args.device,
        vllm_ascend_version=args.vllm_ascend_version,
    )
    csv_paths = sorted(target_data_dir.rglob("split_qkv_rmsnorm_rope_kernel.csv"))
    if not csv_paths:
        raise FileNotFoundError(
            f"No split_qkv_rmsnorm_rope_kernel.csv found under {target_data_dir}"
        )

    total_rows = 0
    for csv_path, row_index, row in iter_csv_rows(
        target_data_dir,
        "split_qkv_rmsnorm_rope_kernel.csv",
    ):
    ...
    print(
        "Processed "
        f"{total_rows} split_qkv_rmsnorm_rope_kernel rows from {len(csv_paths)} "
