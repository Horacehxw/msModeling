...
Purpose:
  ...
  perf_database/data/{device}/vllm_ascend/{version}/MatMulV3.csv,
...
def run_row(csv_path, row_index: int, row: dict[str, str]) -> None:
    ...
    if len(input_shapes) != 2 or len(input_formats) != 2 or len(input_dtypes) != 2:
        raise ValueError("MatMulV3 expects exactly two inputs")
...
def main() -> None:
    ...
    csv_paths = sorted(target_data_dir.rglob("MatMulV3.csv"))
    if not csv_paths:
        raise FileNotFoundError(f"No MatMulV3.csv found under {target_data_dir}")

    total_rows = 0
    for csv_path, row_index, row in iter_csv_rows(target_data_dir, "MatMulV3.csv"):
        run_row(csv_path, row_index, row)
        total_rows += 1

    print(
        f"Processed {total_rows} MatMulV3 rows from {len(csv_paths)} csv file(s) "
