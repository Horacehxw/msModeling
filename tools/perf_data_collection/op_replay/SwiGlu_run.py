...
Purpose:
  ...
  perf_database/data/{device}/vllm_ascend/{version}/SwiGlu.csv,
  rebuild input tensors from the recorded shapes, formats, and dtypes,
  then execute torch_npu.npu_swiglu() with dim fixed to -1.

Usage:
  python tensor_cast/performance_model/perf_database/op_run/SwiGlu_run.py ^
...
def build_argparser():
    return build_standard_argparser(
        ...
        usage_examples=[
            "py -3 tensor_cast/performance_model/perf_database/op_run/SwiGlu_run.py "
            "--device ATLAS_800_A3_752T_128G_DIE --vllm-ascend-version 0.13.0",
            "python tensor_cast/performance_model/perf_database/op_run/SwiGlu_run.py "
...
def main() -> None:
    ...
    total_rows = 0
    for csv_path, row_index, row in iter_csv_rows(target_data_dir, "SwiGlu.csv"):
