...
from .model_config import LinearQuantConfig, LinearQuantType, QuantGranularity, QuantScheme
...
class QuantLinearBase(torch.nn.Module):
    ...
    def __init__(self, linear_layer: torch.nn.Linear, quant_config: LinearQuantConfig):
        ...
        if self.quant_config.quant_type in [LinearQuantType.W8A16, LinearQuantType.W8A8]:
            q_weight_dtype = torch.int8
        elif self.quant_config.quant_type == LinearQuantType.W4A8:
        ...
        # Quantize the weight
        quantized_weight = self.quantize(linear_layer.weight.data, self.quant_config.weight_scale, self.quant_config.weight_offset, q_weight_dtype)

        if self.quant_config.quant_type == LinearQuantType.W4A8:
            # Pack two 4-bit values into a single int8
            quantized_weight = self.pack_int4(quantized_weight, dim=self.quant_config.weight_int4_pack_dim)
    ...
    def quantize(self, tensor: torch.Tensor, scale: torch.Tensor, offset: Optional[torch.Tensor], dtype: torch.dtype) -> torch.Tensor:
    ...
    def dequantize_weight(self) -> torch.Tensor:
        ...
        if self.quant_config.quant_type == LinearQuantType.W4A8:
            unpacked_qweight = self.unpack_int4(self.qweight, self.quant_config.weight_int4_pack_dim)
        else:
    ...
    def unpack_int4(self, packed_tensor: torch.Tensor, dim: int=1) -> torch.Tensor:
        ...
        unpacked = torch.empty(self.out_features, self.in_features, dtype=torch.int8, device=packed_tensor.device)
        if dim == 1:
    ...
    def _calculate_dynamic_qparams(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if cfg.dynamic_quant_dtype == torch.int8:
            ...
            raise ValueError(f"Unsupported dynamic quant dtype: {cfg.dynamic_quant_dtype}")
    ...
    def __repr__(self):
        return (f"QuantLinear(in_features={self.in_features}, out_features={self.out_features}, "
...
class TensorCastQuantLinear(QuantLinearBase):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.activation_scale is not None:
            op = torch.ops.tensor_cast.static_quant_linear_int4 if self.quant_config.quant_type == LinearQuantType.W4A8 else torch.ops.tensor_cast.static_quant_linear
            if self.activation_scale is not None:
                ...
                x = torch.ops.tensor_cast.quantize(x, self.activation_scale, self.activation_offset, out_dtype=torch.int8)
            out = op(
                x,
                qweight,
                self.weight_scale,
            ...
            op = torch.ops.tensor_cast.dynamic_quant_linear_int4 if self.quant_config.quant_type == LinearQuantType.W4A8 else torch.ops.tensor_cast.dynamic_quant_linear
            out = op(
                x,
                qweight,
                self.weight_scale,
