...
class CachingRotaryEmb(torch.nn.Module):
    ...
    """
    def __init__(self, rotary_emb: torch.nn.Module, act_dtype: torch.dtype, max_position_embeddings: int):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, position_ids: torch.Tensor) -> torch.Tensor:
        if self.cos_sin_cache is not None and x.dtype == self.act_dtype:
            return self.cos_sin_cache.index_select(0, position_ids.flatten()).reshape(position_ids.size(0), -1, self.cos_sin_cache.size(-1)).chunk(2, dim=-1)
        else:
