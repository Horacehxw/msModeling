...
class BailingMoeV2AttentionAdapter(torch.nn.Module):
    ...
    def __init__(self, original_attention: torch.nn.Module, attention_by_layers: dict):
        ...
        self.q_proj = torch.nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=self.config.use_qkv_bias)
        self.k_proj = \
            torch.nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=self.config.use_qkv_bias)
        self.v_proj = \
            torch.nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=self.config.use_qkv_bias)
