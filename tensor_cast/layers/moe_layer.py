...
from ..model_config import MoEConfig, MoEKey
...
class MoELayer(torch.nn.Module):
    def __init__(
        ...
        self.gate = gate
        fused_moe_cls = moe_config.fused_moe_cls if moe_config.fused_moe_cls else FusedMoETensorCast
        self.fused_moe = fused_moe_cls(self.moe_config, experts, hidden_act, shared_experts)
...
class FusedMoETensorCast(FusedMoEBase):
    def __init__(
        ...
        self.experts_gate = [
            getattr(expert, moe_config.field_names.gate_proj).weight.data.transpose(0, 1) for expert in experts
        ]
        self.experts_up = [
            getattr(expert, moe_config.field_names.up_proj).weight.data.transpose(0, 1) for expert in experts
        ]
        self.experts_down = [
            getattr(expert, moe_config.field_names.down_proj).weight.data.transpose(0, 1) for expert in experts
        ]
        ...
        if shared_experts:
            shared_experts_gate = getattr(shared_experts, moe_config.field_names.gate_proj).weight.data.transpose(0, 1)
            shared_experts_up = getattr(shared_experts, moe_config.field_names.up_proj).weight.data.transpose(0, 1)
            shared_experts_down = getattr(shared_experts, moe_config.field_names.down_proj).weight.data.transpose(0, 1)
    ...
    def forward(
        ...
        # TODO: call experts and shared_experts explicitly here by distributing tokens evenly or according to some configuration,
        #       then do GMM fusion in the graph pass
