    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            quant_config.linear_configs[name] = get_linear_quant_config(
                quant_type, torch.randn(1), **kwargs,
            )
    return quant_config
...
class TestQuantLinear(unittest.TestCase):
    ...
    def test_pack_int4_raises_error_for_invalid_dim(self):
        ...
        tensor = torch.randint(-8, 8, (OUT_FEATURES, IN_FEATURES), dtype=torch.int8, device=DEVICE)
        dummy_layer = QuantLinearBase(
            self.linear_layer_no_bias,
            get_linear_quant_config(LinearQuantType.W4A8, torch.randn(1))
        )
    ...
    def test_forward_pass_equivalence(self):
        ...
        """
        test_configs = [
            {"quant_type": LinearQuantType.W8A16, "use_bias": True, "w_scheme": "symmetric", "a_scheme": None},
            {"quant_type": LinearQuantType.W8A16, "use_bias": False, "w_scheme": "asymmetric", "a_scheme": None},
            {"quant_type": LinearQuantType.W8A8, "use_bias": True, "w_scheme": "symmetric", "a_scheme": "symmetric"},
            {"quant_type": LinearQuantType.W8A8, "use_bias": True, "w_scheme": "asymmetric", "a_scheme": "asymmetric"},
            {"quant_type": LinearQuantType.W4A8, "use_bias": True, "w_scheme": "symmetric", "a_scheme": "symmetric"},
            {"quant_type": LinearQuantType.W4A8, "use_bias": False, "w_scheme": "symmetric", "a_scheme": "symmetric"},
        ]

        for params in test_configs:
            with self.subTest(**params):
                torch.manual_seed(42)
                linear_layer = torch.nn.Linear(IN_FEATURES, OUT_FEATURES, bias=params["use_bias"]).to(DEVICE, dtype=MODEL_DTYPE)
                ...
                if params["a_scheme"]:
                    config_kwargs["dynamic_quant_scheme"] = QuantScheme.SYMMETRIC if params["a_scheme"] == "symmetric" else QuantScheme.ASYMMETRIC

                config = get_linear_quant_config(params["quant_type"], weight, **config_kwargs)
    ...
    def test_model_quant_base(self, model_id):
        ...
        model_config_with_quant = ModelConfig(ParallelConfig(), get_quant_config(model.model), quant_linear_cls=QuantLinearBase)
        qmodel = TransformerModel(model_id, model_config_with_quant)
        num_qlinear_modules = len(list(module for _, module in qmodel.named_modules() if isinstance(module, QuantLinearBase)))
