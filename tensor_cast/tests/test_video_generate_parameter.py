...
class TestVideoGenerateCfgLogicOnly(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ...
            (
                ...
                False,
                False,
                "CFG disabled + parallel disabled → no extra operations",
            ),
            (
                True,
                False,
                ...
                True,
                False,
                "CFG enabled + parallel disabled → execute extra forward",
            ),
            (
                True,
                True,
                ...
                False,
                True,
                "CFG enabled + parallel enabled → execute cfg all_gather",
            ),
            (
                False,
                True,
                ...
                False,
                False,
                "CFG disabled + parallel enabled → no extra operations",
            ),
        ]
    )
    ...
    def test_cfg_logic_branches(
        ...
        mock_model.forward.return_value = torch.randn(
            1, 16, 10, 10
        )  # Fake forward output
        ...
        self.assertTrue(
            True, f"Test passed [{test_desc}]: core logic branches executed correctly"
        )
