...
**鐗堟湰**: 1.4
**鏃ユ湡**: 2026.3.15
**鍙樻洿**: Phase 1 E2E 闆嗘垚楠岃瘉鍚庢洿鏂帮細閫氫俊鎻掑€笺€乻hape 鍖归厤瑙勫垯銆佽瘎浼版寚鏍囦綋绯?
...
func_name 鈫?op_mapping.yaml 鏌?mapping
  ...
  鈹溾攢 query_mode == "elementwise" 鈫?_lookup_elementwise()
...
**3D鈫?D Flatten Batch 鍖归厤**锛坴1.4 鏂板锛夛細quantize/norm 绫?kernel锛坄AscendQuantV2`, `DynamicQuant`, `RmsNorm`, `AddRmsNormBias`, `AddRmsNorm`锛夌殑 TC dispatch 鍙兘淇濈暀 3D `(B, M, D)` 鏍煎紡锛岃€?profiling CSV 瀛樺偍 2D `(B*M, D)`銆俙_inputs_match` 鍦ㄥ父瑙勫尮閰嶏紙鍚?batch dim=1 strip 鍜?block-padding锛夊け璐ュ悗锛屽杩欎簺 kernel 灏濊瘯 `(B, M, D) 鈫?(B*M, D)` 灞曞钩鍖归厤銆傛瑙勫垯鍖哄埆浜庨€氱敤 `_strip_batch_dim`锛堜粎澶勭悊 B=1锛夛紝浠呭鐗瑰畾 kernel 绫诲瀷鐢熸晥锛岄伩鍏?matmul 绛夌畻瀛愮殑 false positive銆?

**閫氫俊绠楀瓙鍖归厤**锛坄_lookup_comm`锛夛細浠?`args[0]` 璁＄畻 `message_bytes`锛屼粠 `rank_group`锛堜綅缃洜绠楀瓙鑰屽紓锛岃涓嬭〃锛夋帹瀵?`topology_tier`銆傛煡璇㈢瓥鐣ワ細绮剧‘鍖归厤 `(message_bytes, num_devices, topology_tier)` 浼樺厛锛涙湭鍛戒腑鏃舵寜 `message_bytes` 鍋?alpha-beta 妯″瀷鎻掑€?鈥?瀵瑰悓涓€ `(num_devices, topology_tier)` 鍒嗙粍鐨勫叏閮ㄦ暟鎹偣鍋氭渶灏忎簩涔樻嫙鍚?`latency = 伪 + 尾 脳 message_bytes`锛岀粨鏋?clamp 鍒?bracket 涓婁笅鐣屻€傞€氫俊鎻掑€奸粯璁ゅ紑鍚紙涓庤绠楃畻瀛愭彃鍊肩嫭绔嬶級锛屽洜 `message_bytes` 涓鸿繛缁€硷紝绮剧‘鍖归厤鍛戒腑鐜囨瀬浣庛€?
...
| `query_mode` | 鐗规畩鏌ヨ妯″紡 | `attention_special`, `elementwise` |
...
**鏉冮噸杞疆鍖归厤**锛坴1.4 琛ュ厖锛夛細FRACTAL_NZ 鎭㈠涓?ND shape 鍚庯紝瀵?matmul 绫?kernel锛坄_MATMUL_KERNELS` 闆嗗悎锛屽惈 `MatMulV2`, `MatMulV3`, `QuantBatchMatmulV3`, `BatchMatMulV2`, `TransposeBatchMatMul` 绛夛級浠嶉渶灏濊瘯鏉冮噸杞疆鍖归厤 `(K,N) 鈫?(N,K)`銆傛瑙勫垯涓嶉檺浜?ND 鏍煎紡 鈥?FRACTAL_NZ 鎭㈠鍚庣殑 shape 鍚屾牱鍙兘闇€瑕佽浆缃€?

### 4.11 閫愬厓绱犵畻瀛愯緭鍑哄舰鐘跺尮閰?(Elementwise Output-Shape Matching)

瀵逛簬鍐呭瓨甯﹀鍙楅檺鐨勯€愬厓绱犵畻瀛?(Add, Mul, Div), 浣跨敤**杈撳嚭褰㈢姸**鑰岄潪杈撳叆褰㈢姸杩涜鍖归厤:

- **杈撳嚭褰㈢姸纭畾鎬?*: 鏃犺鏍囬噺/鍚戦噺/閫愬厓绱犲箍鎾? 杈撳嚭褰㈢姸鐩稿悓
- **dtype 鏉惧紱鍖归厤**: FP32 鈮?BF16 脳 (4/2), 鎸夊瓧鑺傛瘮缂╂斁寤惰繜 (`_dtype_byte_size()`)
- **閫傜敤鏉′欢**: `Duration 鈭?output_elements 脳 bytes_per_element / memory_bandwidth`
- **缃俊搴?*: dtype 鐩稿悓 = 1.0; dtype 涓嶅悓 (缂╂斁鍚? = 0.9; 鎻掑€?+ dtype 缂╂斁 = 0.6

鍦?`op_mapping.yaml` 涓爣璁?`query_mode: elementwise`銆備笌 `tc_input_count` 浜掓枼銆?
...
### 7.5 璇勪及鎸囨爣浣撶郴锛坴1.4 鏂板锛寁1.4.1 鎵╁睍 M4-M6锛?

鍏眰鎸囨爣浣撶郴锛屼粠绮楀埌缁嗐€佷粠绠楀瓙璁℃暟鍒板欢杩熷姞鏉冿細

| 鎸囨爣 | 鍒嗗瓙 | 鍒嗘瘝 | 鎺掗櫎 zc | 鎮茶 | 铻嶅悎 | 璁＄畻鏂瑰紡 | 鐢ㄩ€?|
|------|------|------|:---:|:---:|:---:|---------|------|
| **M1**: Raw Op-Count HR | HIT 璋冪敤娆℃暟 | 鎬昏皟鐢ㄦ鏁?| 鈥?| 鈥?| 鈥?| 鍦ㄧ嚎 | Debug锛屽悜鍚庡吋瀹?|
| **M2**: Fused Op HR | HIT 鍞竴 func_name 鏁?| 鎵€鏈夊敮涓€ func_name 鏁?| 鈥?| 鉁?| 鉁?| 鍦ㄧ嚎 | **GO/NO-GO 鍒ゅ畾** |
| **M3**: Fused Op HR (涓嶅惈 zc) | 鍚?M2 鎺掗櫎 zero_cost | 鍚?M2 鎺掗櫎 zero_cost | 鉁?| 鉁?| 鉁?| 鍦ㄧ嚎 | 鐪熷疄璁＄畻瑕嗙洊 |
| **M4**: Per-Shape Match HR | HIT 鍞竴 (func, shape) 鏁?| 鎵€鏈夊敮涓€ (func, shape) 鏁?| 鉁?| 鈥?| 鈥?| 鍦ㄧ嚎 | Shape 缂哄彛璇婃柇 |
| **M5**: Simulated Latency Coverage | HIT ops 鐨?analytic 寤惰繜涔嬪拰 | 鎵€鏈?ops 鐨?analytic 寤惰繜涔嬪拰 | 鈥?| 鈥?| 鈥?| 鍦ㄧ嚎 | 浠跨湡瑙嗚寤惰繜瑕嗙洊鐜?|
| **M6**: Empirical E2E Ratio | HIT ops 鐨?empirical (microbench) duration 涔嬪拰 (鍏ㄦā鍨?replay) | step_trace_time / N_forward_passes (鍗曟 forward pass) | 鈥?| 鈥?| 鈥?| 鍗婄绾?| **楠屾敹鏍囧噯锛歁6=1.0 瀹岀編锛?.85鈥?.15 涓?Phase 3 鐩爣** |

#### 鎸囨爣璇︾粏瀹氫箟

**M1鈥揗3**: 绠楀瓙璁℃暟灞?

- M1 鎸夋瘡娆?`process_op()` 璋冪敤璁℃暟锛岃 zero_cost ops 铏氬
- M2/M3 鎸夊敮涓€ `func_name` 璁℃暟锛屽簲鐢ㄦ偛瑙傝鍒欏拰铻嶅悎鍒嗙粍
- **鎮茶瑙勫垯**: 鍚屼竴 `func_name` 鑻ユ湁 N 娆?HIT + M 娆?MISS锛堜笉鍚?shape锛夛紝鏁翠釜绠楀瓙璁′负 1 MISS
- **铻嶅悎鍒嗙粍**: DFC锛坧ermute_tokens + grouped_matmul脳2 + unpermute_tokens + all_to_all脳2锛? 1 铻嶅悎 op锛汳LAPO銆丮LA銆丮C2 鍚岀悊銆傚叏閮ㄦ垚鍛?HIT 鎵嶇畻 HIT

**M4**: Shape 璇婃柇灞?

- 姣忎釜鍞竴鐨?`(func_name, input_shape_tuple)` 鐙珛璁℃暟
- 涓嶅簲鐢ㄦ偛瑙傝鍒欙紝涓嶅簲鐢ㄨ瀺鍚堝垎缁勶紝鎺掗櫎 zero_cost
- 鐢ㄤ簬瀹氫綅鍏蜂綋缂哄け鍝簺 shape 鐨勬暟鎹紝鎸囧 microbenchmark 鏁版嵁閲囬泦

**M5**: 浠跨湡寤惰繜瑕嗙洊灞?

- 鍒嗗瓙鍒嗘瘝鍧囦娇鐢?**analytic (Roofline) 寤惰繜**浣滀负鏉冮噸
- 鏈川鏄?M3 鐨勫欢杩熷姞鏉冪増鏈細楂樺欢杩熺畻瀛愭潈閲嶅ぇ锛屼綆寤惰繜杈呭姪绠楀瓙鏉冮噸灏?
- 鍦ㄧ嚎璁＄畻锛屼笉闇€瑕佸閮ㄦ暟鎹?

**M6**: Empirical E2E Prediction Ratio锛坴1.5 閲嶆柊瀹氫箟锛?

```
M6 = Empirical_HIT_total / Real_per_forward_pass
```

- 璺戜竴娆?TC `--performance-model profiling --export-metrics report.json` 寰楀埌 empirical HIT duration
- **鍒嗗瓙** = HIT ops 鐨?empirical (microbench CSV) 寤惰繜涔嬪拰锛屼箻浠?replay_multiplier 寰楀埌鍏ㄦā鍨?replay 鎬婚噺銆?*涓嶅惈 analytic fallback**锛圡ISS ops 涓嶈鍏ワ級
- **鍒嗘瘝** = `step_trace_time.csv` 鐨?`(Computing + Comm_NotOverlapped) / N_forward_passes`
  - `step_trace_time.csv` 鐨?`Step` 鍒椾负绌烘椂锛岃仛鍚堜簡鏁翠釜 profiling 绐楀彛鐨勬墍鏈?forward passes
  - `N_forward_passes` 浠?`kernel_details.csv` 浼扮畻锛歈wen3 鐢?`FIA_count / 64`锛孌Sv3 鐢?`DFC_count / 58`
  - 涔熷彲閫氳繃 `--n-forward-passes` 鎵嬪姩鎸囧畾
- M6 = 1.0 琛ㄧず瀹岀編棰勬祴锛汳6 > 1 琛ㄧず empirical 楂樹及锛坢icrobench isolation effect锛夛紱M6 < 1 琛ㄧず瑕嗙洊涓嶈冻锛圡ISS ops 鏈础鐚級
- `*AicpuKernel` 鏉＄洰涓?AICPU dispatch wrapper锛屼粎鐢ㄤ簬璇婃柇锛堟帓闄わ級

**涓轰粈涔堝彧鐪?empirical-only锛堜笉鍚?analytic fallback锛夛紵**
- Analytic fallback 鐢?Roofline 妯″瀷浼扮畻 MISS ops 寤惰繜锛屽叾绮惧害鍙楅檺浜庨€氫俊妯″瀷銆佷笉鍙娴嬬殑 kernel fusion 绛夊洜绱?
- 灏?analytic fallback 璁″叆 M6 浼氭贩娣?empirical 鏁版嵁璐ㄩ噺"鍜?analytic 妯″瀷鍑嗙‘搴?涓や釜涓嶅悓闂
- M6 鐨勭洰鐨勬槸璇勪及 **宸叉湁 empirical 鏁版嵁鐨勮川閲忓拰瑕嗙洊搴?*锛歁6 < 1 璇存槑瑕嗙洊涓嶈冻锛堥渶瑕佹洿澶?microbench 鏁版嵁锛夛紝M6 > 1 璇存槑 microbench 鏁版嵁鍋忛珮锛坕solation vs real workload锛?

**M5 vs M6**: M5 浠?TC 浠跨湡瑙嗚鐪嬶紙鏉冮噸=analytic Roofline锛屽湪绾匡級锛孧6 浠?empirical 棰勬祴 vs NPU 鐪熷疄鍗曟 forward pass 鐪嬶紙鍗婄绾匡級銆侻5 鍚?analytic fallback锛孧6 涓嶅惈銆?

#### Phase 鐩爣

- Phase 1: M1鈥揗3 鎸囨爣浣撶郴寤虹珛 鉁?
- Phase 2: M3 > 50%, M4/M5 寤虹珛 鉁?
- Phase 3: M5 > 80%, **0.85 鈮?M6 鈮?1.15**锛埪?5% 浠ュ唴锛?
...
### v1.3.1 鈫?v1.4 (2026.3.15)

**鏂板**:
- 搂4.2: 閫氫俊 alpha-beta 妯″瀷鎻掑€硷紙least-squares fit锛岄粯璁ゅ紑鍚級
- 搂4.2: 3D鈫?D Flatten Batch 鍖归厤瑙勫垯锛坬uantize/norm 绫?kernel锛?
- 搂4.9: FRACTAL_NZ 鎭㈠鍚庢潈閲嶈浆缃尮閰嶏紙鎵€鏈?matmul 鍙樹綋锛?
- 搂7.5: M1-M5 浜斿眰璇勪及鎸囨爣浣撶郴 + 鎮茶瑙勫垯 + 铻嶅悎鍒嗙粍

**鍙樻洿**:
- 搂4.2: 閫氫俊鏌ヨ浠?绮剧‘鍖归厤"鏀逛负"绮剧‘鍖归厤浼樺厛 + alpha-beta 鎻掑€?fallback"
- 鐗堟湰: 1.3.1 鈫?1.4

**涓婁笅鏂?*: Phase 1 E2E 闆嗘垚楠岃瘉锛? 鍦烘櫙锛孮wen3-32B + DSv3锛夊畬鎴愬悗锛屽熀浜庨獙璇佸彂鐜扮殑 shape 鍖归厤瑙勫垯鍜岃瘎浼版寚鏍囬渶姹傛洿鏂般€傝瑙?`reports/phase1-e2e-20260314/phase1_e2e_v2_verification_report_zh.md`
