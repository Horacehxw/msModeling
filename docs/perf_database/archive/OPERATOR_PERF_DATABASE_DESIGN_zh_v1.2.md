...
- **鐩爣鍚庣**锛歷llm-ascend 0.13.0 (鍐呴儴闀滃儚)
...
鈹斺攢鈹€ perf_database/                        # 鏂板锛欴ataSource + 鏁版嵁瀛樺偍
    ...
    鈹斺攢鈹€ data/                             # 鎬ц兘鏁版嵁瀛樺偍锛圙it LFS 绠＄悊 .csv锛?
        鈹斺攢鈹€ ATLAS_800_A3_752T_128G_DIE/
            鈹溾攢鈹€ vllm_ascend/v0.13.0/      # 璁＄畻绠楀瓙锛堝拰 vLLM 鐗堟湰缁戝畾锛?
            鈹?  鈹溾攢鈹€ op_mapping.yaml
            鈹?  鈹斺攢鈹€ {KernelType}.csv      # MatMulV2.csv, GroupedMatmul.csv, ... (~14 涓?
            鈹斺攢鈹€ hccl/v8.1.RC1/            # HCCL 閫氫俊锛堝拰 CANN 鐗堟湰缁戝畾锛岃法 vLLM 鐗堟湰澶嶇敤锛?
                鈹溾攢鈹€ comm_config.yaml
                鈹斺攢鈹€ {hcom_op}.csv         # hcom_allReduce_.csv, HcomAllGather.csv, ... (~4 涓?
...
**鍒濆鍖?*锛氬姞杞?`op_mapping.yaml` + 鎸?`communication_data_ref` 鍔犺浇閫氫俊鏁版嵁锛堝鏈夛級銆侰SV 寤惰繜鍔犺浇骞剁紦瀛樸€?
...
```yaml
version: "0.13.0"                              # vLLM-Ascend 鐗堟湰
device: ATLAS_800_A3_752T_128G_DIE
cann_version: "8.1.RC1"
communication_data_ref: "../../hccl/v8.1.RC1/" # 閫氫俊鏁版嵁鐩稿璺緞
...
**褰撳墠鐘舵€?*锛堝熀浜?gitcode/develop 鍒嗘敮鍒嗘瀽锛夛細

| Gap 椤?| gitcode/develop 鐘舵€?| 璇︽儏 | Profiling 鍗犳瘮 |
|--------|---------------------|------|---------------|
...
| split_qkv_rmsnorm_rope | 浠嶅紑鏀?| 鏃犲搴?pass/op | Qwen3 0.5% (64 娆¤皟鐢? |
| KvRmsNormRopeCache | 浠嶅紑鏀?| 鏃犲搴?pass/op | DSV3 0.8% (2501 娆¤皟鐢? |
| MLA 鍒嗚В | 浠嶅紑鏀?| MLA 浠嶄负鍗曚竴 dispatch 鑺傜偣銆俀1 閫氳繃 `composite: true` + `_lookup_composite()` 鍒嗚В鏌ヨ鍏滃簳锛堣 4.2 鑺傦級锛涢暱鏈熼渶 decomposition pass 灏?MLA 鎷嗕负鐙珛瀛?op | DSV3 鐩稿叧 |
| aten.topk / MoeGatingTopK | 浠嶅紑鏀?| 鏃犲尯鍒嗘満鍒?| DSV3 (2378 娆¤皟鐢? |
| MC2 铻嶅悎 | 浠嶅紑鏀?| 鏃犲搴?pass/op | 鈥?|
...
- **MLA**锛?:N 鏄犲皠锛堜竴涓?TC op 瀵瑰簲 TransposeBatchMatMul + FIA锛夛紝闇€ MLA 鍒嗚В pass锛屽綋鍓?fallback to analytic
